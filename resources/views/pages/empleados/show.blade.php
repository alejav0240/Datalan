        ...
        <div class="bg-white rounded-xl employee-card overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-purple-600 to-blue-800 text-white p-6 flex flex-col md:flex-row items-center">
                ...
                <div class="text-center md:text-left">
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-start mt-2">
                        ...
                        @endphp
                        <span class="role-badge {{ $roleColors[$empleado->user->role] ?? 'bg-gray-500' }} text-white mr-2">
                                {{ ucfirst($empleado->user->role) }}
                            </span>
                        <span class="status-badge bg-green-500 text-white">Activo</span>
                    </div>
                </div>
            </div>
            ...
            <div class="border-b">
                <ul class="flex flex-wrap -mb-px">
                    <li class="mr-2">
                        <button data-tab="personal" class="tab-link inline-block py-4 px-4 text-sm font-medium text-center border-b-2 border-transparent rounded-t-lg hover:text-blue-600 hover:border-blue-600 active">
                            <i class="fas fa-user mr-2"></i>Información Personal
                        </button>
                    </li>
                    <li class="mr-2">
                        <button data-tab="professional" class="tab-link inline-block py-4 px-4 text-sm font-medium text-center border-b-2 border-transparent rounded-t-lg hover:text-blue-600 hover:border-blue-600">
                            <i class="fas fa-briefcase mr-2"></i>Información Profesional
                        </button>
                    </li>
                    <li class="mr-2">
                        <button data-tab="trabajos" class="tab-link inline-block py-4 px-4 text-sm font-medium text-center border-b-2 border-transparent rounded-t-lg hover:text-blue-600 hover:border-blue-600">
                            <i class="fas fa-tasks mr-2"></i>Trabajos Asignados
                        </button>
                    </li>
                    <li class="mr-2">
                        <button data-tab="permisos" class="tab-link inline-block py-4 px-4 text-sm font-medium text-center border-b-2 border-transparent rounded-t-lg hover:text-blue-600 hover:border-blue-600">
                ...
                <div id="personal" class="tab-content active">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="info-value">{{ $empleado->user->name }}</div>
                        </div>

                        <div>
                            <div class="info-label">Email</div>
                            <div class="info-value">{{ $empleado->user->email }}</div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="info-value">{{ ucfirst($empleado->estado_civil) }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div id="professional" class="tab-content">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="info-value">{{ ucfirst($empleado->user->role) }}</div>
                        </div>

                        <div>
                            <div class="info-label">Salario</div>
                            <div class="info-value">Bs. {{ number_format($empleado->salario, 2, ',', '.') }}</div>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                @foreach($empleado->permisos as $permiso)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="font-medium text-gray-900">{{ ucfirst($permiso->tipo) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="p-6 bg-gray-50 border-t">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="info-value">{{ $empleado->created_at->format('d M, Y') }}</div>
                    </div>

                    <div>
                        <div class="info-label">Última Actualización</div>
                        <div class="info-value">{{ $empleado->updated_at->format('d M, Y') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 border-t flex justify-between">
                <a href="{{ route('empleados.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg transition flex items-center">
                    <i class="fas fa-arrow-left mr-2"></i>Volver a la lista
                </a>
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('empleados.edit', $empleado->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition flex items-center">
                        <i class="fas fa-edit mr-2"></i>Editar
                    </a>
                    <button class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition flex items-center" onclick="window.print()">
                        <i class="fas fa-print mr-2"></i>Imprimir
                    </button>
                    <form action="{{ route('empleados.destroy', $empleado->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este empleado?');">
                        @csrf
                        @method('DELETE')
